...
// --- MODAL DE EVOLUÇÃO CLÍNICA DE 7 DIAS ---
let graficoEvolucaoInstance = null;

async function abrirEvolucaoPaciente(botao) {
    const card = botao.closest('.patient-card');
    const nomePaciente = card.querySelector('.nome-input')?.value.trim();

    if (!nomePaciente) {
        alert("Preencha o nome do paciente no leito para buscar o histórico.");
        return;
    }

    const modal = document.getElementById('modal-evolucao-paciente');
    const tituloModal = document.getElementById('titulo-modal-evolucao');
    tituloModal.textContent = `📈 Evolução Clínica (7 Dias) - ${nomePaciente.toUpperCase()}`;
    modal.style.display = 'flex';

    // Calcula os últimos 7 dias com base na data selecionada no calendário
    const datasParaBuscar = [];
    const dataBase = new Date(dataSelecionadaStr + 'T00:00:00');

    for (let i = 6; i >= 0; i--) {
        const d = new Date(dataBase);
        d.setDate(dataBase.getDate() - i);
        datasParaBuscar.push(formatarDataChave(d));
    }

    // Busca no Supabase todos os plantões do período calculado
    const { data: historicoPlantoes, error } = await _supabase
        .from('plantoes')
        .select('data_chave, dados_json')
        .in('data_chave', datasParaBuscar);

    const labelsTempo = [];
    const valoresNews = [];

    if (!error && historicoPlantoes) {
        // Ordena cronologicamente os plantões buscados
        historicoPlantoes.sort((a, b) => a.data_chave.localeCompare(b.data_chave));

        historicoPlantoes.forEach(plantao => {
            const dataDiaStr = plantao.data_chave.split('-').reverse().join('/');
            if (Array.isArray(plantao.dados_json)) {
                plantao.dados_json.forEach(p => {
                    if (p.nome && p.nome.trim().toUpperCase() === nomePaciente.toUpperCase()) {
                        if (p.vitais && Array.isArray(p.vitais)) {
                            p.vitais.forEach(v => {
                                const hora = v.hora || "08:00";
                                const inputs = v.inputs || [];
                                
                                // O Escore NEWS / PEWS Total fica armazenado no campo correspondente (último input ou calculado)
                                const newsVal = parseInt(inputs[inputs.length - 3]) || parseInt(inputs[9]) || 0;

                                labelsTempo.push(`${dataDiaStr} ${hora}`);
                                valoresNews.push(newsVal);
                            });
                        }
                    }
                });
    ...
    renderizarGraficoEvolucao(labelsTempo, valoresNews);
}

function fecharEvolucaoPaciente() {
    const modal = document.getElementById('modal-evolucao-paciente');
    if (modal) modal.style.display = 'none';
...
function renderizarGraficoEvolucao(labels, dados) {
    const ctx = document.getElementById('graficoEvolucaoPaciente').getContext('2d');
    ...
    graficoEvolucaoInstance = new Chart(ctx, {
        type: 'line',
        data: {
            labels: labels.length > 0 ? labels : ["Nenhum registro encontrado nos últimos 7 dias"],
            datasets: [{
                label: 'Escore de Risco (NEWS / PEWS)',
        ...
        options: {
            ...
            scales: {
                y: { 
                    beginAtZero: true, 
                    ticks: { precision: 0 },
                    title: { display: true, text: 'Escore' }
                },
                x: {
                    title: { display: true, text: 'Data e Hora da Aferição' }
                }
            },
            plugins: {
